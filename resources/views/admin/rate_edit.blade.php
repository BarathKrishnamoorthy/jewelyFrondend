...
@section('content')
    ...
    <div class="container mx-auto px-4 py-10">
        <b ><a href="/home">Dashboard</a> > <a href="/rate-list">Rate List</a> > <a class="add">Edit</a></b>
    ...
    <form action="{{ route('rate.update', $rate->id) }}" method="POST" enctype="multipart/form-data"
        class="max-w-lg mx-auto bg-white p-8 rounded shadow">
        @csrf
        @method('PUT')
        <div class="mb-6">
            <label for="title" class="block text-gray-700 font-bold mb-2">Title</label>
            <input type="text" name="title" id="title" value="{{ old('title', $rate->title) }}"
        ...
        <div class="mb-6">
            ...
            <input type="text" name="subtitle" id="subtitle" value="{{ old('subtitle', $rate->subtitle) }}"
                class="w-full px-4 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-blue-400">
        </div>
        <div class="mb-6">
            <label for="rate" class="block text-gray-700 font-bold mb-2">Rate</label>
            <input type="text" name="rate" id="rate" value="{{ old('rate', $rate->rate) }}"
                class="w-full px-4 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-blue-400">
        </div>
        <div>
